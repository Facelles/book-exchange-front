      ...
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1 text-sm">
            {user?.email} · {books.length} book{books.length !== 1 ? "s" : ""}{" "}
            listed
          </p>
        </div>
        <button
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-20 bg-slate-800/40 border border-white/5 rounded-2xl animate-pulse"
            />
          ))}
        </div>
      ) : error ? (
          ...
          {books.map((book) => (
            ...
            >
              <div className="w-12 h-16 rounded-lg overflow-hidden bg-slate-700 flex-shrink-0 relative border border-white/10">
                {book.photoUrl && book.photoUrl.startsWith("http") ? (
                  <Image
                    src={book.photoUrl}
                    alt={book.name}
                    fill
