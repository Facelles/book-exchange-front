...
export default function AdminPage() {
  ...
  const [form, setForm] = useState<AddUserForm>({ email: '', password: '', role: 'USER' });
  ...
  const fetchUsers = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch<AdminUser[]>('/api/admin/users', { auth: true });
      setUsers(data);
    } catch (err: unknown) {
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
        {[
          ...
          { label: 'Admins', value: users.filter((u) => u.role === 'ADMIN').length, icon: ShieldCheck, color: 'text-amber-400' },
          { label: 'Regular Users', value: users.filter((u) => u.role === 'USER').length, icon: UserCog, color: 'text-slate-400' },
        ].map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="bg-slate-800/60 border border-white/10 rounded-2xl px-5 py-4">
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-16 bg-slate-800/40 border border-white/5 rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : error ? (
        <div className="text-center py-20">
          <p className="text-red-400">{error}</p>
          <button onClick={fetchUsers} className="mt-4 px-5 py-2 bg-slate-800 hover:bg-slate-700 rounded-xl text-sm text-white transition">
            Retry
          </button>
        </div>
      ) : (
          ...
          {users.length === 0 ? (
            ...
            users.map((u, idx) => (
              ...
              >
                ...
                <div className="col-span-2 text-slate-600 text-xs hidden sm:block">{u.id}</div>
                <div className="col-span-1 flex justify-end">
                  ...
                  >
                    {deleteLoading === u.id ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}
                  </button>
                </div>
              </div>
            ))
